...
public class RemoteNotificationsManager: NSObject {
    ...
    public func unregister() {
        print("Unregistering for remote notifications")
    ...
    public func registerIfNeeded() {
        guard !UIApplication.shared.isRegisteredForRemoteNotifications else {
            print("Skip registering for remote notifications: already registered")
            return
        }
        
        print("The app is not registered for remote notifications: attempt to register with a prompt for user permissions")
